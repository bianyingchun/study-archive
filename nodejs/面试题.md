...
2. 缺点
   ...
   5. 可靠性低，一旦某个环节崩溃，整个系统都崩溃
      原因：单线程
      解决：
      1. nginx 发现代理，负载均衡，开多个进程，绑定多个端口；
...
**PM2 通过命令行CL，使用 RPC 建立 Client(命令行接收端，负责创建守护进程 Daemon，并与 Daemon保持 RPC 连接) 与 Daemon 进程之间的通信，通过 RPC 通信方式，调用 God(负责进程的创建和管理)，从而应用 Node.js 的 cluster.fork 创建子进程的**
