...
- Watcher 订阅者，关联 Observer 和 Compiler,能够订阅并收到属性变化的通知，执行指令绑定的相应操作，更新视图。update 是它的一个方法，用于执行 Compiler 绑定的回调，更新视图。

1. new Vue()首先执行初始化，对 data 执行响应化处理，这个过程发生 Observe 中
2. 同时对模板执行编译，找到其中动态绑定的数据，从 data 中获取并初始化视图，这个过程发生在 Compile 中
3. 同时定义⼀个更新函数 update 和 Watcher，将来对应数据变化时 Watcher 会调用更新函数
